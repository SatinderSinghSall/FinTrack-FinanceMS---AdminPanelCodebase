...
type DeleteFinancialTipModalProps = {
...
export default function DeleteFinancialTipModal({
  financialTip,
  onClose,
  onDeleted,
}: DeleteFinancialTipModalProps) {
  ...
  const financialTipType = financialTip.type || "tip";

  const typeStyle = typeStyles[financialTipType] || typeStyles.tip;
  ...
  const handleDelete = async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Unable to delete financial tip.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="delete-financial-tip-title"
    >
        ...
        <div className="px-6 pb-5 pt-6 sm:px-7">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              ...
              disabled={loading}
              onClick={onClose}
              aria-label="Close delete financial tip dialog"
          ...
          <p className="mt-4 max-w-[390px] text-sm font-medium leading-6 text-zinc-500">
            This will permanently remove the financial tip from FinTrack. Once
            deleted, it cannot be recovered.
          </p>

          {/* FINANCIAL TIP PREVIEW */}
          ...
          <div className="mt-5 overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50/60">
            ...
            <div className="flex items-center justify-between gap-3 border-b border-zinc-200/80 bg-white px-4 py-3">
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-1.5">
                <span
                  className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[9px] font-bold capitalize ${typeStyle.badge}`}
                >
                  <span
                ...
                {financialTip.featured && (
                  <span className="inline-flex items-center gap-1 rounded-full border border-violet-200 bg-violet-50 px-2 py-1 text-[9px] font-bold text-violet-700">
            ...
            <div className="p-4">
              ...
              {financialTip.category && (
                <div className="mt-4">
                  <span
                    className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[9px] font-bold ${
                      categoryStyles[financialTip.category] ||
                      "border-zinc-200 bg-zinc-100 text-zinc-600"
                    }`}
                  >
                    {formatCategory(financialTip.category)}
                  </span>
                </div>
              )}
          ...
          {!error && (
            <div className="mt-4 flex items-start gap-2.5 rounded-2xl border border-amber-200/80 bg-amber-50/60 px-3.5 py-3">
              ...
              <p className="text-[11px] font-semibold leading-5 text-amber-800">
                This action permanently deletes the financial tip and removes it
                from future financial tip results.
              </p>
            </div>
          )}
        </div>
            ...
            >
              {loading ? (
                <>
                  ...
                  <span>Deleting Financial Tip...</span>
                </>
              ) : (
                <>
                  <Trash2 size={15} />

                  <span>Delete Financial Tip</span>
                </>
              )}
            </button>
          </div>
