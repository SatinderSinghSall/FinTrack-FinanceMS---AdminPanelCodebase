...
type ViewFinancialTipModalProps = {
...
const categoryStyles: Record<string, string> = {
  budgeting: "border-blue-200 bg-blue-50 text-blue-700",
  saving: "border-emerald-200 bg-emerald-50 text-emerald-700",
  expenses: "border-amber-200 bg-amber-50 text-amber-700",
  debt: "border-red-200 bg-red-50 text-red-700",
  investing: "border-violet-200 bg-violet-50 text-violet-700",
  "financial-safety": "border-orange-200 bg-orange-50 text-orange-700",
  "money-habits": "border-cyan-200 bg-cyan-50 text-cyan-700",
  goals: "border-indigo-200 bg-indigo-50 text-indigo-700",
};

const formatCategory = (category?: string) => {
  if (!category) return "Money Habits";
...
export default function ViewFinancialTipModal({
  financialTip,
  onClose,
}: ViewFinancialTipModalProps) {
  if (!financialTip) {
    return null;
  }

  const type = financialTip.type || "tip";

  const style = typeStyles[type] || typeStyles.tip;

  const category = financialTip.category || "money-habits";

  const actionEnabled = Boolean(financialTip.action?.enabled);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="view-financial-tip-title"
    >
      <div className="relative flex max-h-[calc(100dvh-24px)] w-full max-w-2xl flex-col overflow-hidden rounded-3xl border border-zinc-200/80 bg-white shadow-[0_30px_80px_-20px_rgba(0,0,0,0.45)] sm:max-h-[calc(100dvh-40px)]">
        ...
        <div className="flex shrink-0 items-start justify-between gap-4 border-b border-zinc-100 bg-white px-5 py-5 sm:px-7 sm:py-6">
          <div className="flex min-w-0 items-start gap-3.5">
            <div className="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-blue-100 bg-blue-50 text-blue-600 shadow-sm">
              <Lightbulb size={21} strokeWidth={1.9} />

              <span className="absolute -bottom-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full border-2 border-white bg-violet-600 text-white">
                <BookOpen size={9} />
              </span>
            </div>
          ...
          <button
            type="button"
            onClick={onClose}
            aria-label="Close financial tip details"
          ...
          <div className="space-y-5">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <h3 className="mt-1.5 break-words text-xl font-extrabold tracking-tight text-zinc-950 sm:text-2xl">
                      {financialTip.title || "Untitled Financial Tip"}
                    </h3>

                    <p className="mt-2 break-words text-sm font-medium leading-6 text-zinc-500">
                      {financialTip.shortDescription ||
                  ...
                  <div className="flex shrink-0 flex-wrap items-center gap-2 sm:flex-col sm:items-end">
                    ...
                    >
                      ...
                      {financialTip.isActive ? "Active" : "Inactive"}
                    </span>

                    {financialTip.featured && (
            ...
            <section className="rounded-2xl border border-zinc-200/80 bg-white shadow-sm">
              ...
              <div className="px-4 py-4 sm:px-5 sm:py-5">
                <p className="whitespace-pre-wrap break-words text-sm font-medium leading-7 text-zinc-700">
                  {financialTip.content || "No content provided."}
                </p>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <DetailItem
                  icon={<Power size={16} />}
                  label="Status"
                  value={financialTip.isActive ? "Active" : "Inactive"}
                  badge
                  badgeClass={
                    financialTip.isActive
                      ? "border-emerald-200 bg-emerald-50 text-emerald-700"
                ...
                <DetailItem
                  icon={<Star size={16} />}
                  label="Featured"
                  value={financialTip.featured ? "Featured" : "Standard"}
                  badge
                  badgeClass={
                    financialTip.featured
                ...
                <DetailItem
                  ...
                  label="Start Date"
                  value={formatDate(financialTip.startDate)}
                />

                <DetailItem
                  icon={<Clock size={16} />}
                  label="End Date"
                  value={formatDate(financialTip.endDate)}
                />
              </div>
            </section>
              ...
              <div className="p-4 sm:p-5">
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <DetailItem
                    icon={<Pencil size={16} />}
                    label="Button Label"
                    value={financialTip.action?.label || "—"}
                    compact
                  />
                  ...
                  <div className="sm:col-span-2">
                    <DetailItem
                      icon={<Link size={16} />}
                      label="App Route"
                      value={financialTip.action?.route || "—"}
                      mono
                      compact
                    />
                  </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 gap-3">
                <DetailItem
                  icon={<Hash size={16} />}
                  label="Financial Tip ID"
                  value={financialTip._id}
                  mono
                />
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <DetailItem
                    icon={<Plus size={16} />}
                    label="Created At"
                    value={formatDate(financialTip.createdAt)}
                  />

                  <DetailItem
                    icon={<History size={16} />}
                    label="Last Updated"
                    value={formatDate(financialTip.updatedAt)}
                  />
                </div>
              </div>
            </section>
        ...
        <div className="shrink-0 border-t border-zinc-100 bg-zinc-50/70 px-5 py-4 sm:px-7">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="hidden text-[10px] font-medium text-zinc-400 sm:block">
              Showing complete financial tip configuration.
            </p>
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {badge ? (
          ...
          <p
            className={`mt-1 break-words text-xs font-bold text-zinc-800 ${
